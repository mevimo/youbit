...
def encode_local(
    ...
    with CONSOLE.status(f"Encoding '{str(input)}'...", spinner="bouncingBall"):
        ...
        with Encoder(input) as encoder:
            encoder.encode(
                path=output, overwrite=overwrite, ecc=ecc, bpp=bpp, crf=crf, res=res
            )
...
def encode_upload(
    ...
    video_title: str = typer.Argument(
        ..., help="The title to use for the uploaded YouTube video."
    ),
    ecc: int = opt_encode_ecc,
    res: str = opt_encode_res,
    bpp: int = opt_encode_bpp,
    ...
    with Encoder(input) as encoder:
        res = tuple(map(int, res.split("x")))
        encoder.encode(ecc=ecc, bpp=bpp, crf=crf, res=res)
