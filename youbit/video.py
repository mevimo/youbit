...
class VideoEncoder:
    ...
    def feed(self, arr: ndarr_1d_uint8) -> None:
        ...
        try:
            ...
            raise ValueError(
                f"The length of the given array must be a multiple of the framesize, in this case {self.stream.width*self.stream.height} (= {self.stream.width} * {self.stream.height})."
            ) from e
        for framearr in arr:
...
class VideoDecoder:
    ...
    def __init__(self, vid: Union[str, Path]) -> None:
        ...
        if self.stream.codec_context.framerate == 1:
            warnings.warn(
                f"Video passed to {type(self).__name__} has a framerate of 1. This probably means the video did not go though YouTube. Beware that this decoder will only work on videos that have gone through YouTube's own compression."
            )
